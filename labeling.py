...
def compute_customer_features_cutoff(df: pd.DataFrame,
...
def label_repeat_purchase(df: pd.DataFrame, cutoff_date: pd.Timestamp,
    ...
    features["Repeat"] = features["Customer ID"].isin(returning_ids).astype(int)
...
def future_spend_target(df: pd.DataFrame, cutoff_date: pd.Timestamp,
    ...
    features["FutureSpend"] = features["Customer ID"].map(future_spend).fillna(0)
...
if __name__ == "__main__":
    ...
    print(f"\nMedian future spend (returning customers only): "
          f"{targets.loc[targets['FutureSpend'] > 0, 'FutureSpend'].median():.2f}")
